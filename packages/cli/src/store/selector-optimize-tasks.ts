...
export const selectorOptimizeTasksNotPending = (store: Store): Record<string, OptimizeTask> =>
  Object.fromEntries(
    Array.from(store.indexState.entries()).filter(
      ([_, task]) => task.state.type !== TypeOptimizationState.Pending,
    ),
...
const selectorOptimizeTasksFulfilledAll = (
  ...
  return Object.fromEntries(
    Array.from(store.indexState.entries()).filter(
      ([key, task]) => keys.includes(key) && task.state.type === TypeOptimizationState.Fulfilled,
    ),
...
export const selectorOptimizeTasksFulfilled = (
  ...
  Object.fromEntries(
    map(
      ...
      (array) => {
        const value = [...array].sort(({ state: { cost: a } }, { state: { cost: b } }) => a - b)[0]

        // @ts-expect-error unable to type JSONType
        const key = objectHash(value.options)

        const task = store.indexState.get(key) as OptimizeTask<OptimizationStateFulfilled>
